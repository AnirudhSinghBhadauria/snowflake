...
-- Creating a stored procedure for checking if a particular table has duplicates or not,
CREATE OR REPLACE PROCEDURE check_table_duplicates(
    table_name VARCHAR,
    columns_to_check VARCHAR
)
RETURNS TABLE (duplicate_record_count INTEGER, duplicate_columns VARIANT)
LANGUAGE SQL
AS
$$
DECLARE
    duplicate_query VARCHAR;
    result_columns VARCHAR;
BEGIN
    -- Prepare the list of columns for SELECT and GROUP BY
    result_columns := REPLACE(columns_to_check, ' ', '');

    -- Construct the dynamic SQL for duplicate check
    duplicate_query := '
    WITH duplicate_check AS (
        SELECT ' || columns_to_check || ', 
        COUNT(*) AS occurrence_count
    FROM ' || table_name || '
    GROUP BY ' || columns_to_check || '
    HAVING COUNT(*) > 1
    )
    SELECT 
        COUNT(*) AS duplicate_record_count,
        OBJECT_CONSTRUCT(' || 
        LISTAGG('''' || TRIM(value) || ''', ' || TRIM(value), ', ') 
        WITHIN GROUP (ORDER BY value) || ') AS duplicate_columns
    FROM duplicate_check';

    RETURN TABLE(
        WITH result AS (
            EXECUTE IMMEDIATE :duplicate_query
        )
        SELECT * FROM result
    );
END;
